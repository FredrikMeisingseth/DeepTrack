...
def generate_image(image_parameters):
    ...
    if len(particle_center_x_list) > 0:
    ...
    for sample_size, iteration_number in zip(sample_sizes, iteration_numbers):
        for iteration in range(iteration_number):
            ...
            for image_number, image, image_parameters in image_generator():
                ...
                ### CHANGED FROM ORIGINAL DEEPTRACK
                if len(image_parameters['Particle Center X List']) > 0:
            ...
            if not (iteration % int(verbose ** -1)):
                print('Sample size %6d   iteration number %6d   MSE %10.2f px^2   MAE %10.2f px   Time %10.2f ms' % (
                    sample_size, iteration + 1, mse, mae, iteration_time * 1000))
...
def plot_learning_performance(training_history, number_of_timesteps_for_average=100, figsize=(20, 20)):
    ...
    plt.semilogy(
        convolve(training_history['MSE'], ones(number_of_timesteps_for_average) / number_of_timesteps_for_average,
                 mode='valid'), 'r')
    ...
    plt.semilogy(
        convolve(training_history['MAE'], ones(number_of_timesteps_for_average) / number_of_timesteps_for_average,
                 mode='valid'), 'r')
...
def predict(network, image):
    """ Predict position of particle in a image using the deep learning network.
...
def plot_prediction(image, image_parameters, predicted_position, figsize=(15, 5)):
    ...
    if len(particle_center_x_list) > 0:
        ...
        plt.plot(predicted_position_r * sin(arange(-pi / 30, 2 * pi, pi / 30)),
                 predicted_position_r * cos(arange(-pi / 30, 2 * pi, pi / 30)), ':', color='#e6661a')
    plt.xlabel('y (px)', fontsize=16)
    plt.ylabel('x (px)', fontsize=16)
...
def particle_positions(
    ...
    if particle_number > 0:
        ...
        return [target_particle_center_x], [target_particle_center_y]
